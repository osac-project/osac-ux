/**
 * flow: provider-admin
 * route: /provider/templates/bm/:id/edit
 *
 * BM template spec (title, description) is defined externally in osac-app /
 * the AAP controller — this page shows it read-only. The default host type
 * is an exception: it's just a reference to a resource the Provider Admin
 * already owns/prices (see /provider/host-types), so it's editable here.
 * The other editable surface is the Billing tab: published state,
 * BillableComponents (pricing), and the per-tenant allow-list (sharing).
 */
...
import {
  ...
  usePatchBareMetalInstanceTemplate,
...
const BACK = '/provider/templates?tab=bm';

export const ProviderBmTemplateFormPage = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();

  const { data: existing, isLoading: loadingExisting } = useBareMetalInstanceTemplate(id ?? '');
  ...
  const existingSpecDefaults = existing as unknown as
    | { specDefaults?: { hostType?: string } }
    | undefined;
  ...
  useEffect(() => {
    if (existing && !hydrated) {
      ...
      setHostTypeId(existingSpecDefaults?.specDefaults?.hostType ?? '');
      setHydrated(true);
    }
  }, [existing, existingSpecDefaults, hydrated]);

  const patch = usePatchBareMetalInstanceTemplate();
    ...
    try {
      await patch.mutateAsync({
        ...
        patch: {
          ...
          specDefaults: { ...existingSpecDefaults?.specDefaults, hostType: hostTypeId },
        } as never,
      });
      navigate(BACK);
    } finally {
  ...
  if (loadingExisting) {
    return (
      <PageSection hasBodyWrapper={false}>
        <Spinner aria-label={t('Loading BM template')} />
      </PageSection>
    );
  }
  ...
  return (
    <>
      ...
      <PageSection hasBodyWrapper={false}>
        <Form onSubmit={handleSubmit} style={{ maxWidth: '560px' }} id="bmt-form">
          {activeTab === 'billing' && (
          ...
          {activeTab === 'details' && (
            <>
              ...
              <FormGroup
                label={t('Default host type')}
                fieldId="bmt-host-type"
                style={{ marginTop: '1rem' }}
              >
                <FormSelect
                  id="bmt-host-type"
                  value={hostTypeId}
                  onChange={(_e, v) => setHostTypeId(v)}
                >
                  <FormSelectOption value="" label={t('— None —')} />
                  {hostTypes.map((ht) => (
                    <FormSelectOption key={ht.id} value={ht.id} label={hostTypeName(ht)} />
                  ))}
                  {hostTypeId && !hostTypes.some((ht) => ht.id === hostTypeId) && (
                    <FormSelectOption value={hostTypeId} label={hostTypeId} />
                  )}
                </FormSelect>
                <Content component="small" style={{ color: 'var(--pf-t--global--color--200)' }}>
                  {t('Editable — references a Host Type you manage under Host Types.')}
                </Content>
              </FormGroup>
            </>
          )}
          ...
          {patch.error && (
            <Alert
              variant="danger"
              title={t('Failed to update BM template')}
              isInline
              style={{ marginTop: '1rem' }}
            >
