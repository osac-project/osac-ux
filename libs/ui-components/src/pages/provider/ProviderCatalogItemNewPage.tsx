/**
 * flow: provider-admin
 * route: /provider/catalog-studio/new
 * route: /provider/catalog/new
 */
...
import { useCreateMaaSCatalogItem } from '../../api/v1/maas-catalog-item';
...
export const ProviderCatalogItemNewPage = () => {
  ...
  const isCatalogRoute =
    location.pathname.startsWith('/provider/catalog/') ||
    location.pathname === '/provider/catalog/new';
  const backRoute = isCatalogRoute
    ? '/provider/catalog'
    : '/provider/catalog-studio?tab=catalog-items';
  const breadcrumbParent = isCatalogRoute ? 'Global Catalog' : 'Catalog Studio';
  const breadcrumbParentPath = isCatalogRoute
    ? '/provider/catalog'
    : '/provider/catalog-studio?tab=catalog-items';

  const [itemType, setItemType] = React.useState<ItemType>('vm');
  const [typeOpen, setTypeOpen] = React.useState(false);
  const [templateId, setTemplateId] = React.useState(prefillTemplate);
  const [templateOpen, setTemplateOpen] = React.useState(false);
  const [title, setTitle] = React.useState('');
  const [description, setDescription] = React.useState('');
  const [published, setPublished] = React.useState(true);
  const [pricePerHour, setPricePerHour] = React.useState('');
  const [osLabel, setOsLabel] = React.useState('');
  const [archLabel, setArchLabel] = React.useState('');
    ...
    <>
      <PageSection hasBodyWrapper={false}>
        <Stack hasGutter>
          <Breadcrumb>
            <BreadcrumbItem>
              <Button variant="link" isInline onClick={() => navigate(breadcrumbParentPath)}>
                {breadcrumbParent}
              </Button>
            </BreadcrumbItem>
            <BreadcrumbItem isActive>Create catalog item</BreadcrumbItem>
          </Breadcrumb>
      ...
      <PageSection hasBodyWrapper={false}>
        <Form onSubmit={handleSubmit} style={{ maxWidth: '640px' }} id="cci-create-form">
          ...
          {mutationError && (
            <Alert variant="danger" title="Failed to create catalog item" isInline>
          ...
          <ActionGroup>
            <Button
              variant="primary"
              type="submit"
              form="cci-create-form"
              isLoading={isPending}
              isDisabled={isPending || !title.trim()}
            >
              Create
            </Button>
            <Button variant="link" onClick={() => navigate(backRoute)} isDisabled={isPending}>
              Cancel
            </Button>
          </ActionGroup>
        </Form>
